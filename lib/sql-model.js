      ...
      _this.beforeSave({
        callback: function(err) {
          ...
          _this.Model.connection.query(statement, bindVars, function(queryErr, result) {
            if (config.logQuery || _options.debug) { fileLogger.log('Query: ' + statement + ' - binds: ' + bindVars); }
  ...
  erase(options) {
    ...
    return new Promise((resolve, reject) => {
      ...
      this.Model.connection.query(sql, sqlParams, function(err, result) {
        if (err) return response(err);
        if (config.logQuery || _options.debug) { fileLogger.log('Query: ' + sql + ' - binds: ' + sqlParams + ' - result length: ' + ( result.affectedRows || 0 )); }
  ...
  static update(options) {
    ...
    return new Promise((resolve, reject) => {
      ...
      this.connection.query(sql, sqlParams, function(err, result) {
        if (config.logQuery || _options.debug) { fileLogger.log('Query: ' + sql + ' - binds: ' + sqlParams); }
        if (err) return response(err);
