...
class SqlModel extends MasterModel {
      ...
      _this.beforeSave({
        callback: function(err) {
          ...
          _this.Model.connection.query(statement, bindVars, function(queryErr, result) {
            if (config.mysql.logQuery || _options.debug) { fileLogger.log('Query: ' + statement + ' - binds: ' + bindVars); }
  ...
  erase(options) {
    ...
    return new Promise((resolve, reject) => {
      ...
      this.Model.connection.query(sql, sqlParams, function(err, result) {
        if (err) return response(err);
        if (config.mysql.logQuery || _options.debug) { fileLogger.log('Query: ' + sql + ' - binds: ' + sqlParams + ' - result length: ' + ( result.affectedRows || 0 )); }
  ...
  static update(options) {
    ...
    return new Promise((resolve, reject) => {
      ...
      if (Object.keys(_options.data).length < 1) {
        return response('No data found');
      }

      if (_options.conditionMandatory && Object.keys(_options.where).length < 1) {
        return response('No conditions found');
      }

      const sql = SqlHelper.buildStaticUpdateQueryStr(_this, _options.where, sqlParams, _options.data);
      if (!sql) {
        return response('Where statement not found for update');
      }

      // Query sql
      this.connection.query(sql, sqlParams, function(err, result) {
        if (config.mysql.logQuery || _options.debug) { fileLogger.log('Query: ' + sql + ' - binds: ' + sqlParams); }
        if (err) return response(err);

/*        if (result.affectedRows < 1) {
          return response('Record does not exist');
        }*/
        return response(null, result.affectedRows);
      });
    });
  }
  ...
  static destroy(options) {
    ...
    return new Promise((resolve, reject) => {
      ...
      const sql = SqlHelper.buildDestroyQueryStr(_this, sqlParams, _options.where);
      if (!sql) {
  ...
  static query(options) {
    ...
    return new Promise((resolve, reject) => {
      ...
      if (_options.debug) {
        fileLogger.log('Query: ' + sqlStr + ' - binds: ' + sqlParams);
      }

      // Execute query
      _this.connection.query(sqlStr, sqlParams, function(err, results, fields) {
        if (config.mysql.logQuery || _options.debug) {
