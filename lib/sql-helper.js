...
class SqlHelper {
   ...
   * @returns {string}
   */
  static buildDestroyQueryStr(Model, sqlParams, whereObj) {
    const wheres = [];
    Object.keys(whereObj).forEach(function(key) {
      sqlParams.push(whereObj[key]);
      wheres.push('`' + key + '` = ?');
    });
    ...
    // Build sql
    return 'DELETE FROM `' + Model.table + '` WHERE ' + wheres.join(' AND ') + ';';
  }
  ...
  static buildUpdateQueryStr(modelInstance, bindvars, saveData) {
    ...
    if (modelInstance.get(modelInstance.Model.primaryKey)) {
      ...
      statement.push(setStatements.join(','));
  ...
  /**
   * Update without a model instance
   * @param modelInstance
   * @param bindvars
   * @param saveData
   * @returns {string}
   */
  static buildStaticUpdateQueryStr(Model, whereCondition, bindvars, saveData) {
    const statement = [];
    const setStatements = [];
    const modelInstance = new Model(); // TODO is this correct? We are only making a new instance to make sure we can get the last modified field

    // Set the last modified field
    if (modelInstance._options.lastModifiedField) {
      if (!saveData[modelInstance._options.lastModifiedField]) {
        saveData[modelInstance._options.lastModifiedField] = SqlHelper.generateTimeStamp(new Date());
      }
    }
    ...
    Object.keys(whereCondition).forEach((key, i) => {
      whereStatement.push('`' + key + '` = ? ');
      bindvars.push(whereCondition[key]);
    });

    return statement.join('') + whereStatement.join(' AND ');
  }
  ...
  static buildSaveDataFromFields(modelInstance) {
    ...
    // Only set created for new records
    if (!objId && modelInstance._options.createdField) {
  ...
  static buildQuerySql(Model, sqlParams, queryOptions) {
    ...
    const _options = Object.assign({}, {
      ...
      whereNotLike: {}, // Example: {{name:"%test", x: "_test"}, y: ['%test1', '%test2%', 'test']}
      single: false,
      count: false,
      include: [],
      fromToDate: {},
    ...
    const notLikeCon = []; // WhereLike statement
    ...
    if (Object.keys(_options.where).length > 0 || Object.keys(_options.whereNot).length > 0 || ('field' in _options.fromToDate) || Object.keys(_options.whereLike).length > 0 || _options.whereOr.length > 0) { // || Object.keys(_options.whereOr).length > 0
    ...
    // Start after a certain date (Example: WHERE CrtStamp < DATE_SUB(NOW(), INTERVAL 5 MINUTE);))
    SqlHelper.pushAfterDateFilter(Model, andList, _options.afterDate);
    ...
    // WhereNotLike  statement
    SqlHelper.toWhereNotLikeSqlList(Model, _options.whereNotLike, notLikeCon, sqlParams);
    ...
    // Join notLikeCon
    if (notLikeCon.length > 0) {
      andList.push(notLikeCon.join(' AND '));
    }
  ...
  static toWhereNotSqlList(Model, whereNot, list, sqlParams) {
    Object.keys(whereNot).forEach(function(key, i) {
      ...
      if (Array.isArray(whereNot[key])) {
        ...
        if (whereNot[key] === null) {
          list.push(Model.table + '.' + key + ' IS NOT ? ');
        } else {
          list.push(Model.table + '.' + key + ' != ? ');
        }
      }
   ...
   * @param sqlParams
   */
  static toWhereNotLikeSqlList(Model, whereLike, list, sqlParams) {
    Object.keys(whereLike).forEach(function(key, i) {
      if (Array.isArray(whereLike[key])) {
        whereLike[key].forEach(function(val) {
          list.push(Model.table + '.' + key + ' NOT LIKE ? ');
          sqlParams.push(val);
        });
      } else {
        list.push(Model.table + '.' + key + ' NOT LIKE ? ');
   ...
   * options.fromToDate: {
  			interval: 1, // Number
  			intervalType: DAY, // DAY | MINUTE etc
  			field: null //db field
  	}
   */
  static pushAfterDateFilter(Model, sqlList, afterDate) {
    if (('field' in afterDate)) {
      afterDate.interval = afterDate.interval ? afterDate.interval : 1;
      afterDate.intervalType = (afterDate.intervalType ? afterDate.intervalType : 'MINUTE');
      sqlList.push(Model.table + '.' + afterDate.field + ' < DATE_SUB(NOW(), INTERVAL ' + afterDate.interval + ' ' + afterDate.intervalType + ')');
    }
  }
    ...
    include.forEach(function(name, i) {
      ...
      associations.forEach(function(ass, assIndex) {
        ...
        if (ass.Model.table === JoinModel.table) {
          ...
          const modelPkCapitalized = SqlHelper.capitalize(Model.primaryKey || 'id');
          const modelPk = Model.primaryKey || 'id';

          const joinModelPkCapitalized = SqlHelper.capitalize(JoinModel.primaryKey || 'id');
          const joinModelPk = JoinModel.primaryKey || 'id';

          switch (ass.type) {
            case 'hasOne':

              // queryJoins.push('LEFT JOIN ' + JoinModel.table + ' ON ' + JoinModel.table + '.id = ' + Model.table + '.' + JoinModel.table + SqlHelper.capitalize('id'));
              queryJoins.push('LEFT JOIN ' + JoinModel.table + ' ON ' + JoinModel.table + '.' + joinModelPk + ' = ' + Model.table + '.' + JoinModel.table + joinModelPkCapitalized);
              break;
            case 'hasMany':

              // queryJoins.push('LEFT JOIN ' + JoinModel.table + ' ON ' + JoinModel.table + '.' + Model.table + 'Id = ' + Model.table + '.id');
              queryJoins.push('LEFT JOIN ' + JoinModel.table + ' ON ' + JoinModel.table + '.' + Model.table + modelPkCapitalized + ' = ' + Model.table + '.' + modelPk);
              break;
          }
        }
      });
  ...
  static buildJoinModel(currentModel, rec) {
    currentModel.getAssociations().forEach(function(ass, i) {
      ...
      // Check if field exists. If not lets add it as a sync field
      var field = currentModel.getField(ass.name);
      if (!field) {
        currentModel.addField(ass.Model.createField({
          name: ass.name,
          sync: false
        }));
      }
